...
if __name__ == '__main__':
    ...
    for e, element_index in enumerate(sampled_indices[sample_num]):
        ...
        if data['post']['rewrite_prompts_correct'][-1]:#model learnt the fact correctly
            correct_facts += 1
        
        case_num = data['case_id']
        all_cases[case_num] = {
                        'edit_success': data['post']['rewrite_prompts_correct'][-1],
                        'edit_num': e+1, 
    ...
    for num_edits, hist_folder in sorted_history:
        ...
        for filename in os.listdir(hist_path):
            if '.json' in filename:
                ...
                if not data['post']['rewrite_prompts_correct'][-1] and all_cases[case_num]['edit_success'] and 'forget_index' not in all_cases[case_num]:
    ...
    plt.ylabel('Percentage Facts Forgotten', fontsize=20)
    #plt.xlim(0, 1200)
