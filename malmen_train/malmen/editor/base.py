...
class BaseEditor:
    ...
    def run(self, train_loader: DataLoader, valid_loader: DataLoader):
        ...
        model_name = {"/data/akshat/models/gpt-j-6b": "gpt-j", "/data/akshat/models/gpt2": "gpt2"}
        
        torch.save(self.net.state_dict(), f"/data/christinefang/unified-model-editing/malmen_train/malmen/checkpoints/{model_name[self.config.model.name_or_path]}_{self.config.editor.name}_{str(self.config.data.n_edits)}_net.pth")
        torch.save(self.opt.state_dict(), f"/data/christinefang/unified-model-editing/malmen_train/malmen/checkpoints/{model_name[self.config.model.name_or_path]}_{self.config.editor.name}_{str(self.config.data.n_edits)}_opt.pth")