...
class CounterFactDataset(Dataset):
    def __init__(
        ...
        **kwargs,
        ...
        qna_prompt_filename = data_dir / 'counterfact_qna.json'
        with open(qna_prompt_filename, 'r') as openfile:
            # Reading from json file
            json_object = json.load(openfile)


        for case_id in range(len(json_object)):
            self.data[case_id]['generation_prompts'].append(json_object[case_id]['qna_question'])
