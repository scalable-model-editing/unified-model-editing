...
class BaseEditor:
    ...
    def __init__(
        ...
        if config.editor.load_checkpoint:
            ...
            self.net.load_state_dict(torch.load(f"/data/christinefang/unified-model-editing/malmen_train/malmen/checkpoints/gpt-j_malmen_512_net.pth"))
            self.opt.load_state_dict(torch.load(f"/data/christinefang/unified-model-editing/malmen_train/malmen/checkpoints/gpt-j_malmen_512_opt.pth"))

            # self.net.load_state_dict(torch.load(f"/data/christinefang/unified-model-editing/malmen/checkpoints/{config.model.name_or_path}_{config.editor.name}_{str(config.data.n_edits)}_net.pth"))
            # self.opt.load_state_dict(torch.load(f"/data/christinefang/unified-model-editing/malmen/checkpoints/{config.model.name_or_path}_{config.editor.name}_{str(config.data.n_edits)}_opt.pth"))
    ...
    def valid(self, loader: DataLoader):
        ...
        for tuples in tqdm(loader, desc = "Valid", ncols = 100):
            ...
            ### CALCULATE paraphrase, generalization succ metrics
            for k, s in zip(
    ...
    def model_editing(self, loader: DataLoader):
        for tuples in tqdm(loader, desc = "Train", ncols = 100):
    ...
    def run(self, train_loader: DataLoader, valid_loader: DataLoader):
        ...
        torch.save(self.net.state_dict(), f"/data/christinefang/unified-model-editing/malmen/checkpoints/{self.config.model.name_or_path}_{self.config.editor.name}_{str(self.config.data.n_edits)}_net.pth")
        torch.save(self.opt.state_dict(), f"/data/christinefang/unified-model-editing/malmen/checkpoints/{self.config.model.name_or_path}_{self.config.editor.name}_{str(self.config.data.n_edits)}_opt.pth")
